 ...
 *
 * Three changes to the setup code (the shaders themselves are untouched):
 *
 *  1. Compiling and linking are wrapped. The published version throws out of
 *     the effect when a shader fails to compile, which takes the page with it
 *     — and a driver that won't compile this is exactly the case worth
 *     surviving. A failure now leaves a blank canvas and one console warning.
 *     It also checks the link status, which was unchecked: a failed link draws
 *     nothing and reports nothing.
 *
 *  2. The loop stops when the tab is hidden. It ran a full-rate
 *     requestAnimationFrame forever otherwise.
 *
 *  3. Reduced motion renders one frame instead of animating.
 *
 * Two and three are the same lesson this codebase already paid for on its
 * background shader, which stuttered on iOS until it stopped rendering
 * frames nobody was looking at.
 */
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // Unchecked in the original: a failed link draws nothing and says nothing.
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        throw new Error(gl.getProgramInfoLog(program) ?? "program link error")
      }
      gl.useProgram(program)
    } catch (e) {
      // A driver that won't compile this is exactly the case worth surviving:
      // leave the canvas blank rather than taking the page down with it.
      console.warn("SiriWave: shader unavailable —", e)
