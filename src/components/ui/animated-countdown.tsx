...
export function AnimatedCountdown({
  ...
  // Replaces the published `mounted` flag, which did the same job. One state
  // instead of two, and it is set from the same place the time is, so
  // "we have a real reading" and "here is the reading" can never disagree.
  ...
  const completed = ticked && !isStatic && isFinished(displayTimeLeft);
  ...
  React.useEffect(() => {
    ...
    // body: a synchronous state write there cascades renders, and this project
    // lints against it. The first reading still lands before paint, so the
