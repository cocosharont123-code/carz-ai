...
/**
 * A sign-in card: 3D tilt, travelling light beams, glass panel.
 *
 * Read the note before wiring this up. It is a *presentational* form — the
 * submit handler waits two seconds and resets, it does not authenticate — and
 * this app has no password authentication to wire it to. Auth is Google-only
 * through NextAuth (src/auth.ts registers one provider), there is no
 * credentials provider and no password is stored anywhere. Its "Sign up" and
 * "Forgot password?" links point at /signup and /forgot-password, neither of
 * which exists in this app.
 *
 * Four changes from the published source, all to compile and to keep this
 * project's lint baseline:
 *   - `focusedInput` is typed, which removes the two `as any` casts it needed.
 *   - The unused `mousePosition` state and the unused `handleSubmit` are gone;
 *     the form uses its own inline handler.
 *   - `useEffect` was imported and never used.
 *   - The apostrophe in "Don't have an account?" is escaped.
 */
