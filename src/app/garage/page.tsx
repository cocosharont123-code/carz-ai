...
export default function GaragePage() {
  ...
  const rarest = cars.reduce<Car | null>((best, c) => (!best || c.rarityScore > best.rarityScore ? c : best), null);
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-3xl px-5 py-10">
        ...
        <p className="mt-1 text-muted-foreground">Every car you&apos;ve spotted, all in one place.</p>
        ...
        {loading ? (
          ...
          <div className="mt-6 rounded-3xl border border-sky-500/40 bg-sky-500/10 p-6 text-center">
            <h3 className="text-lg font-bold">Garage is being set up</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              Your garage needs the database connected. Check back soon!
            </p>
          </div>
        ) : !signedIn ? (
          <div className="mt-6 rounded-3xl border border-white/[0.06] bg-card p-8 text-center backdrop-blur-xl">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Your spotted cars are saved to your account.
            </p>
            <button
              onClick={() => signIn("google", { callbackUrl: "/garage" })}
              ...
              {cars.map((c) => (
                <div key={c.id} className="overflow-hidden rounded-2xl border border-white/[0.06] bg-card">
                  <div className="relative aspect-square w-full bg-white/[0.04]">
                    ...
                    {c.rarityScore >= 70 && (
                      <span className="absolute right-1.5 top-1.5 rounded-full bg-amber-400/90 px-1.5 py-0.5 text-[10px] font-bold text-black">
                        RARE
                      </span>
                    )}
                  </div>
